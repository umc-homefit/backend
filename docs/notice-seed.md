...
- 서울 청년안심주택 6개 단지
- `[TEST]` 표시가 있는 공고 12건
  - 추가모집 6건, 일반모집 필터 확인용 1건, 분석 등급 데모 5건
...
- 동일 DB에서 두 번 실행해도 단지 6개, 공고 12개와 각 자식 12개를 유지한다.

## 분석 등급 데모

`[TEST] 분석 데모` 공고는 로그인한 테스트 계정의 조건 프로필을 아래 값으로 바꾼 뒤 분석 요청하면 등급을 재현할 수 있다. 공통 조건은 월소득 100만 원, 자산 0원, 무주택 여부 등 분석에 필요한 값을 정상 입력하는 것이다.

| 공고                | 재현용 보유 현금 | 재현용 월소득 | 기대 등급                          |
| ------------------- | ---------------: | ------------: | ---------------------------------- |
| `DEMO-HIGH`         |       1,000만 원 |      100만 원 | `HIGH`                             |
| `DEMO-MEDIUM`       |       5,000만 원 |      100만 원 | `MEDIUM`                           |
| `DEMO-LOW`          |       2,500만 원 |      100만 원 | `LOW`                              |
| `DEMO-NOT-ELIGIBLE` |       1,000만 원 |      100만 원 | `NOT_ELIGIBLE` (소득 조건 불충족)  |
| `DEMO-NEED-CHECK`   |       1,000만 원 |      100만 원 | `NEED_CHECK` (자동 판정 불가 조건) |
