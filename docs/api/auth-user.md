...
| 409 | 이미 존재하는 이메일 (`AUTH409`) — 사전 중복 조회 및 동시 요청 레이스 컨디션 모두 포함 |

> **Memo**: 회원가입 성공 시 응답 구조(`AuthResultDto`)는 그대로지만, 내부적으로 `UserProfile`이 함께 생성되고 `nickname`에 랜덤 값(형용사+팀원이름+숫자 조합, 예: `우아한주드38`)이 자동으로 채워진다. 즉 가입 직후 `GET /users/me/profile`을 호출하면 이제 `nickname`이 `null`이 아니라 항상 값이 있는 상태로 조회된다. User와 프로필은 하나의 원자적 쓰기(Prisma nested create)로 생성되어, 실패 시 계정만 남는 반쪽 상태는 발생하지 않는다.
...
> **Memo**: 신규 소셜 가입(`isNewUser: true`)일 때도 이메일 가입과 동일하게 `UserProfile`이 함께 생성되고 랜덤 닉네임이 자동 부여된다. 기존 소셜 계정으로 재로그인하는 경우(`isNewUser: false`)에는 프로필을 다시 만들지 않고 기존 값을 그대로 유지한다.
